...
const validDataSubmitted = configurations => {
  ...
  configurations.config.forEach(config => {
    ...
    if (config.value > baseConfig.maxValue || (baseConfig?.minValue && config.value < baseConfig.minValue)) errors.push(errorMessage);
  });
...
const validSaveConfiguration = (ctx, next) => {
  ...
  if (!configurations.userId) return ctx.badRequest({ errors: [dictionary.configuration.find(c => c.name === 'USER_ID_EMPTY')] });
